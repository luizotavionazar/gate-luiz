...
- `views/` — Um Vue SFC por página (`Login`, `Cadastro`, `Conta`, `RecuperarSenha`, `RedefinirSenha`, `CancelarRecuperacao`, `Setup`, `VerificacaoEmail`). `ContaView` inclui botão "Painel de Controle" visível apenas ao admin mestre do PermLuiz (verificado via `GET /me/admin` em `VITE_PERM_LUIZ_URL`); ao clicar, abre o PermLuiz com o JWT no fragment da URL (`#token=<jwt>`).
...
| POST | `/auth/recuperacao/iniciar` | Pública | Inicia recuperação de senha — body: `{email}` envia código por e-mail ou `{telefone}` envia via WhatsApp/SMS (exige telefoneVerificado e Twilio configurado); canal telefone também envia e-mail de alerta com botão de cancelamento |
| POST | `/auth/recuperacao/redefinir` | Pública | Redefine senha com código válido — body: `{email|telefone, codigo, novaSenha}` |
| POST | `/auth/recuperacao/cancelar` | Pública | Cancela (invalida) um token de recuperação via `{tokenCancelamento}` — UUID único por token, enviado no e-mail de alerta quando a recuperação é iniciada por telefone |
