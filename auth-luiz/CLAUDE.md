...
- **Recuperação de senha:** Limitada por IP via `ControleRecuperacaoSenha`. Usa código numérico de 6 dígitos enviado por e-mail; o código é hasheado antes de armazenar (`TokenRecuperacaoSenha`). A redefinição ocorre em um único passo: `POST /auth/recuperacao/redefinir` recebe `{email, codigo, novaSenha}` — não há endpoint de validação prévia do código. Proteção contra brute-force: após 5 tentativas erradas o código é bloqueado. A limpeza de tokens expirados é feita pelo `TokenRecuperacaoSenhaExpiracaoService`.
